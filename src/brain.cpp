...
void Brain::init(std::mt19937& rng) {
    // Xavier uniform initialisation: range ±sqrt(1/fanIn)
    ...
    for (auto& w : weightsInputHidden)  w = distIH(rng);
    for (auto& w : weightsHiddenOutput) w = distHO(rng);

    // Zero-init biases is standard and correct
...
// Pure neuroevolution — no broken REINFORCE.
// To train: run N agents, rank by total reward, keep top K, mutate to refill.
Brain Brain::mutate(float mutationRate, std::mt19937& rng) const {
    std::uniform_real_distribution<float> dist(-1.0f, 1.0f);
    Brain child = *this;

    for (auto& w : child.weightsInputHidden)  w += dist(rng) * mutationRate;
    for (auto& w : child.weightsHiddenOutput) w += dist(rng) * mutationRate;
    for (auto& b : child.biasHidden)          b += dist(rng) * mutationRate;
    for (auto& b : child.biasOutput)          b += dist(rng) * mutationRate;
